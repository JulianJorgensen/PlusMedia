...
class Marquee extends React.Component {
  ...
  render() {
    let {title, intro, header, subheader, transitionSpeed, bgImage, bgVideo, poster, className, split, scroll} = this.props;
    ...
    if (split){
      ...
      return (
        <div className={`${styles.marquee} ${className}`} style={{backgroundImage: `url(${bgImage})`, opacity: marqueeOpacity}}>
          ...
          {bgVideo ? <video src={bgVideo} poster={poster} autoPlay loop /> : ''}
        </div>
      )
    }
  }
